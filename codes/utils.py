...
def plot_AUC_ROC(fprs,tprs,method_names,fig_dir, dataset, model_name, num):
    # set font style
    font={'family':'serif'}
    ...
    # print auc  
    print("****** ROC AUC ******")
    print("CAVEAT : AUC of V-GAN with 8bit images might be lower than the floating point array (check <home>/auc/{}/{}/auc_roc_{}.npy)".format(dataset,model_name, num))
...
def plot_AUC_PR(precisions, recalls, method_names, fig_dir, dataset,model_name, num):
    # set font style
    font={'family':'serif'}
    matplotlib.rc('font', **font)
    ...
    print("CAVEAT : AUC of V-GAN with 8bit images might be lower than the floating point array (check <home>/auc/{}/{}/auc_pr_{}.npy)".format(dataset, model_name, num))
    for index in indices:
        if method_names[index]!='CRFs' and method_names[index]!='2nd_manual':
...
def get_imgs(target_dir, augmentation, img_size, dataset, mask=False):
    ...
    if augmentation:
        ...
        angle = 15 if dataset == 'DRIVE' else 8
        for angle in tqdm(range(angle,360,angle), desc='angle...'):
