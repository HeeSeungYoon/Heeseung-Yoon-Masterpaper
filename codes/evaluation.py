...
for dataset in datasets:
    ...
    for result in all_results:
        if ("mask" not in result):    #skip mask and ground truth
            ...
            # visualize results
            if "V-GAN" in result or "EXPERIMENT" in result or "DRIU" in result or "1st_manual" in result:
    ...
    utils.plot_AUC_ROC(fprs, tprs, methods, curve_dir, os.path.basename(dataset), model_name, num)
    utils.plot_AUC_PR(precs, recalls, methods, curve_dir, os.path.basename(dataset), model_name, num)